...
def cal_S_of_t2(L, t0, Mz_array, beta, EF, steps, dt, phi=0):
...
def vis_S_of_t2(L, Mz_array, steps, dt):
    file_name = f"S_{model_name}_{state_name}_L_{L}_EF_{EF}_Mz_{Mz_array[0]}_{Mz_array[-1]}_steps_{steps}_dt_{dt}"
    data = np.load("data/" + file_name + ".npz")
    S_array = data["S_array"]
...
if __name__ == "__main__":
    ...
    for i in range(1, 8):
        EF = L // 8 * i
        start_time = time.time()
        cal_S_of_t2(L, t0, Mz_array, beta, EF, steps, dt, phi)
        vis_S_of_t2(L, Mz_array, steps, dt)
        end_time = time.time()
        print(f"i = {i}, elapsed time: {end_time - start_time:.1f} s")